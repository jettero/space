...
def generate(x=50, y=50, rsz='2d4+1', rsparse='1d10+3',
             ...
             seed_budget=None, inertia='5', turn_prob='10', max_steps='180',
             # loop creation (higher chance reduces dead-ends)
             loop_chance='18', loop_max_len='14',
...
def prune_deadends(a_map, keep_door_depth=3, min_prune_len=2):
    ...
    def degree(p):
        x,y = p
        c = a_map[x,y]
        return sum(1 for d in ((x+1,y),(x-1,y),(x,y+1),(x,y-1)) if a_map.in_bounds(*d) and isinstance(a_map[d], Cell))
    ...
    while changed:
        ...
        tips = [(i,j) for (i,j), c in list(a_map) if isinstance(c, Cell) and degree((i,j)) == 1 and not is_room_cell((i,j))]
        if tips:
